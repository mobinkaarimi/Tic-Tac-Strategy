...
const toneClasses: Record<NonNullable<GlassButtonProps["tone"]>, string> = {
  ...
  cyan: "bg-gradient-to-r border-sky-300/80 from-sky-200/85 shadow-[0_12px_35px_-18px_rgba(56,189,248,0.7)] text-slate-900 to-sky-400/85 via-cyan-300/85  dark:border-sky-500/40 dark:from-sky-500/50 dark:text-white dark:to-sky-500/50 dark:via-cyan-400/60",
  neutral:
...
export const GlassButton = forwardRef<HTMLButtonElement, GlassButtonProps>(
  function GlassButton(
    { children, variant = "primary", tone = "cyan", className, ...props },
    ref
  ) {
    const base =
      "duration-200 ease-out font-semibold inline-flex items-center justify-center px-4 py-2 rounded-full text-sm transition-all";
    const focusStyles = "  focus:outline-none focus:ring-2 focus:ring-white/60";
    ...
    const ghost =
      "backdrop-blur-md bg-white/30 border text-slate-800  hover:scale-[1.02]  focus:ring-offset-0  dark:border-white/15 dark:bg-white/10 dark:text-white";
