...
const markClasses: Record<Player, string> = {
...
const BaseCell: React.FC<CellProps> = ({ index, value, isWinning, disabled, onClick }) => {
  const occupied = value !== null
  const interactable = !disabled && !occupied

  const markClass =
    value === null ? 'text-slate-600  dark:text-slate-500' : markClasses[value]
  ...
  const lightCellClass =
    'backdrop-blur-md border-neutral-300 bg-neutral-100/85 focus:ring-neutral-300/70 focus:ring-offset-neutral-100/80 shadow-[inset_0_1px_0_rgba(255,255,255,0.7),0_8px_28px_-24px_rgba(51,65,85,0.45)] text-slate-800'
  const darkCellClass =
    '  dark:border-neutral-800 dark:bg-neutral-900/80 dark:focus:ring-neutral-700 dark:focus:ring-offset-neutral-950 dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.05),0_8px_30px_-24px_rgba(0,0,0,0.7)] dark:text-slate-100'
