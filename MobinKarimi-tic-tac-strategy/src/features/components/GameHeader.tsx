...
> = {
  X: {
    ...
    text: "bg-clip-text bg-gradient-to-br drop-shadow-[0_0_12px_rgba(56,189,248,0.75)] from-sky-300 text-transparent to-sky-500 via-cyan-400",
  },
  O: {
    active:
    ...
    text: "bg-clip-text bg-gradient-to-br drop-shadow-[0_0_12px_rgba(251,191,36,0.75)] from-amber-300 text-transparent to-orange-600 via-orange-400",
  },
};
        ...
        <div className="flex flex-wrap gap-3 items-center">
          <div className={pillClasses("X")}>
            ...
            {currentPlayer === "X" && !isOver && (
              <span className="flex h-2.5 items-center justify-center relative w-2.5">
                <span
                  className={`absolute animate-ping h-3 inline-flex opacity-70 rounded-full w-3 ${playerPalette.X.dot}`}
                />
                <span
                  className={`relative h-2.5 inline-flex rounded-full w-2.5 ${playerPalette.X.dot}`}
                />
              </span>
            )}
          </div>
          <div className={pillClasses("O")}>
            ...
            {currentPlayer === "O" && !isOver && (
              <span className="flex h-2.5 items-center justify-center relative w-2.5">
                <span
                  className={`absolute animate-ping h-3 inline-flex opacity-70 rounded-full w-3 ${playerPalette.O.dot}`}
                />
                <span
                  className={`relative h-2.5 inline-flex rounded-full w-2.5 ${playerPalette.O.dot}`}
                />
              </span>
            )}
          </div>
